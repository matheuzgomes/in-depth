...
**Special method lookup** bypasses the normal instance-attribute lookup (`__getattribute__`) in most cases. Instead, CPython looks up the method **on the type**, not the instance (`Objects/typeobject.c`, `_Py_lookup_special`). This is documented in the [Python data model](https://docs.python.org/3/reference/datamodel.html) (Ch. 3): "The implementation works through a prototype chain, looking for the method in the type hierarchy."
...
<Callout type="info">
  Ramalho (Fluent Python, 2nd ed., Ch. 11) describes Python's protocol mechanism as "duck typing at the syntax level." The language does not require formal interface declarations. If a class implements <code>__len__</code> and <code>__getitem__</code>, it is a sequence. If it implements <code>__enter__</code> and <code>__exit__</code>, it is a context manager. This design reduces ceremony compared with formal interface systems found in Java or Go.
</Callout>
...
- [CPython source: Objects/typeobject.c](https://github.com/python/cpython/blob/main/Objects/typeobject.c)
- [CPython source: Python/bltinmodule.c](https://github.com/python/cpython/blob/main/Python/bltinmodule.c)
- Fluent Python, 2nd ed., Ch. 11: Special Methods and Protocols — covers the protocol-driven design, the type-level lookup mechanism, and the full protocol table
