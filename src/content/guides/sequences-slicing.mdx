...
Slicing is Python's compact notation for half-open ranges over sequences. It is not just syntax sugar: it encodes an interval model, drives copy behavior, and determines whether your code allocates a new object or mutates an existing sequence in place.

<SLabel>Core answer</SLabel>

Python slices use half-open intervals: `seq[start:stop]` includes `start` and excludes `stop`. That choice makes boundaries composable and length arithmetic simple.
...
The excluded `stop` is why `seq[:x]` and `seq[x:]` partition a sequence without overlap or gaps. That is the real reason the convention survives: it keeps slicing algebra easy in production code.
