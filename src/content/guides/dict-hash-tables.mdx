...
A bad hash function does not break correctness, but it destroys performance. Fluent Python (Ramalho, 2022, Ch. 3) demonstrates this by noting that `hash(1) == hash(1.0) == hash(True)` all produce the same hash because Python's `int`, `float`, and `bool` types are all part of the numeric tower and compare equal across types. That means `{1: "int", 1.0: "float"}` has only one key — the second insertion overwrites the first.
...
That is not a bug: it follows from the numeric-comparison contract in the data model. Two values that compare equal must share the same dict slot.
...
<Callout type="info">
  The compact dict was introduced in CPython 3.6 and became a language guarantee in Python 3.7. Ramalho (Fluent Python, 2nd ed., Ch. 3) notes that this change saved roughly 58% memory for a typical dict compared with the old combined-table layout, and it also gave the preservation of insertion order as a side effect of the dense-entries array.
</Callout>
...
- Fluent Python, 2nd ed., Ch. 3: Dictionaries and Sets — covers the compact dict, key-sharing, and the numeric-tower hash collision pattern
