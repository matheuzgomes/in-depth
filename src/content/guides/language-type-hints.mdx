Type hints are design-time contracts for readers, editors, and static analyzers. They do not enforce runtime correctness unless your code adds validation or dispatch behavior that consumes the annotations.
...
Parameterized built-in generics such as `list[int]` are for annotation meaning, not runtime `isinstance` checks.
