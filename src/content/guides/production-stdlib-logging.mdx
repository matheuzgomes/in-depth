The standard library [`logging`](https://docs.python.org/3/library/logging.html) package is a hierarchical event pipeline: loggers create records, filters decide what continues, handlers emit, and formatters render. Production logging quality depends less on calling `log.info(...)` and more on understanding propagation, lazy formatting, and failure boundaries.
