...
This guide covers the chapter-21 boundary layer: what async can and cannot do, why CPU-bound work still starves the event loop, and how to type async protocols without collapsing everything into unhelpful `Any`.
...
- it does not make CPU-bound Python bytecode run in parallel inside one event-loop thread
...
- it does not replace process-based CPU parallelism
- it does not compensate for a blocking library hidden behind async syntax
