...
**The evaluation loop** (`Python/ceval.c`). CPython's bytecode interpreter is a massive switch statement inside `_PyEval_EvalFrameDefault`. Each opcode is a `case` label. The loop:

1. reads the next instruction from `f->f_lasti` (the instruction pointer in the current frame)
2. decodes the opcode and its argument
3. dispatches to the matching `case`
4. advances `f_lasti` and repeats
...
**The frame execution model.** Each function call creates a frame (`PyFrameObject` in `Objects/frameobject.c`). The frame holds:

- `f_locals` — the fast-locals array (not a dict, just a C array of `PyObject*`)
- `f_globals` — a reference to the module's global dict
- `f_builtins` — a reference to the builtins dict
- `f_lasti` — the instruction pointer (offset into the bytecode)
- `f_valuestack` — the evaluation stack, a contiguous array of `PyObject*`
...
<Callout type="info">
  Ramalho (Fluent Python, 2nd ed., Ch. 23) describes bytecode as "the C level you can read from Python." The <code>dis</code> module is the bridge between high-level syntax and the interpreter's execution model. He recommends using it diagnostically — to answer a specific question about execution shape — rather than as a routine optimization tool.
</Callout>
...
- Fluent Python, 2nd ed., Ch. 23: Attribute Descriptors and the `__getattr__` protocol (includes bytecode-level discussion of attribute access patterns)
