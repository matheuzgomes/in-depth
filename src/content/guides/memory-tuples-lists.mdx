...
<Callout type="info">
  Ramalho (Fluent Python, 2nd ed., Ch. 2) calls this the "tuple as record" vs "tuple as immutable list" duality. The tuple-as-record pattern uses positional unpacking and field names via <code>NamedTuple</code>. The tuple-as-immutable-list pattern is what most programmers reach for when they need an unchangeable sequence. The CPython allocation optimization (one contiguous block) is a bonus, not the primary design goal.
</Callout>
...
- Fluent Python, 2nd ed., Ch. 2: An Array of Sequences — covers list overallocation, tuple layout, and the "tuple as record" pattern
