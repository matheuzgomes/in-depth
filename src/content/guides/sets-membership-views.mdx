...
The set base object (`216` bytes for an empty set on CPython 3.12) is much larger than a dict (`64` bytes) because the set object includes inline hash table arrays even at minimum size. Fluent Python (Ramalho, 2022, Ch. 3) notes that sets make this tradeoff deliberately: they optimize for the membership workload, not for compact empty-state storage.
...
<Callout type="info">
  Ramalho (Fluent Python, 2nd ed., Ch. 3) emphasizes that <code>frozenset</code> is not just an immutable set — it is hashable, which means it can be used as a dict key or set element. This is its primary practical advantage over mutable <code>set</code>, not just the immutability.
</Callout>
...
- Fluent Python, 2nd ed., Ch. 3: Dictionaries and Sets — covers the set hash table, frozenset hashability, and set algebra patterns
