...
The GIL, or **global interpreter lock**, is one of the most important runtime constraints in CPython. It affects how threads behave, why some code scales across cores and some does not, and why "Python threading is slow" is both partly true and often poorly explained.
...
The GIL is implemented in `Python/ceval.c` (the main evaluation loop) and `Python/pystate.c` (the thread state management). In the standard CPython build (`Py_GIL_DISABLED` not defined), the evaluation loop acquires the GIL before executing bytecode and releases it during blocking operations.

**Acquisition model.** CPython uses a periodic-check approach. The GIL is not released after every bytecode instruction. Instead, the evaluation loop checks a `_Py_atomic_load` of the `gil_drop_request` flag at configurable intervals. The default check interval is determined by `sys.setswitchinterval()` (default: 5 milliseconds on CPython 3.12). When a waiting thread sets this flag, the running thread releases the GIL after completing its current "tick" of bytecode execution.

**Release points.** The GIL is explicitly released in these situations (`Python/ceval.c` and `Python/thread.c`):
...
That interval is not a promise of exact fairness or exact timing. It is a runtime tuning knob for how often Python threads are given a chance to switch, and the actual scheduling still depends on the operating system and on whether the current thread reaches a point where switching can happen.

**Per-interpreter GIL ([PEP 684](https://peps.python.org/pep-0684/), Python 3.12+).** Starting in Python 3.12, CPython supports `Py_NewInterpreter()` creating sub-interpreters that each have their own GIL. This is implemented in `Python/pystate.c` — the GIL is stored per-interpreter in the `PyInterpreterState` struct, so threads running in different interpreters do not contend for the same lock. `InterpreterPoolExecutor` (Python 3.14) provides a high-level API for this.

<Callout type="info">
  Ramalho (Fluent Python, 2nd ed., Ch. 19) emphasizes distinguishing "concurrency" (dealing with many things at once) from "parallelism" (doing many things at once). The GIL makes this distinction concrete: threads provide concurrency for I/O-bound work but not parallelism for CPU-bound Python bytecode. Ramalho recommends processes, not threads, for CPU-bound parallelism in the regular CPython build.
</Callout>

<SLabel>What the GIL does and does not guarantee</SLabel>

The GIL protects CPython runtime internals. It does **not** mean:

- your high-level operations are always atomic
- your threaded program is race-free
- compound operations on shared data are automatically correct
- your API design can ignore locks and ownership

For example, `x += 1` is not a magical atomic business operation just because the process has a GIL. It still represents multiple interpreter-level steps and can still be logically racy when multiple threads coordinate incorrectly.

This lines up with the newer glossary wording around atomic operations and race conditions: Python only guarantees atomicity where it is explicitly documented.

<Callout type="warn">
  The GIL is not a substitute for synchronization in threaded application code. It protects the interpreter from many forms of corruption; it does not prove your shared-state logic is correct.
</Callout>
...
- Fluent Python, 2nd ed., Ch. 19: Concurrency Models in Python — covers the GIL, threads, processes, and when each model wins
