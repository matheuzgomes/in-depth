...
<SLabel>How it works at runtime</SLabel>

At a high level, a thread running Python code needs the interpreter's attached thread state, which in regular builds effectively means it must hold the GIL for that interpreter.

In practice, this means:

- one thread runs Python bytecode
- another Python thread may be ready, but it cannot run bytecode until the current thread yields the lock or blocks
- while a thread is in blocking I/O or certain native-code sections, the GIL can be released so another thread can run
...
The interpreter also has a thread switch interval:
...
- making that safe requires substantial internal runtime changes
