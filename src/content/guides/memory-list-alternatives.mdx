`list` is the default Python sequence because it is flexible, indexed, and easy to mutate. It is not the right data structure when the real workload is numeric storage, queue operations at the left edge, or zero-copy binary slicing.
