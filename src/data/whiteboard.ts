...
const TOPIC_DETAILS: Record<string, TopicBoardDetails> = {
  ...
  "sequences-pattern-matching": {
    boardIntro: "A sequence pattern is a structural branch that rejects mismatched shapes before the body runs. It gives shape its own branch logic instead of simulating that with an if-chain.",
    whatItIs: [
    ...
    howItWorks: [
      ...
      "Cases are tried in order. The first matching shape wins, so ordering carries semantic meaning.",
    ...
    whenToUseIt: [
      ...
      "Prefer normal conditionals for simple boolean flags. Pattern matching earns its place when structure is the question.",
      "Order cases from most specific to most general, with a bare `case _:` fallback at the bottom.",
    ],
    takeaways: [
      "Case order is semantic because the first match wins.",
      "Names bind after a successful match.",
  ...
  "language-identity-equality": {
    boardIntro: "You have probably debugged this: `a == c` is True while mutating `a` leaves `c` alone. Mutating `a` also changes `b`. That contrast is the practical difference between `==` and `is`.",
    whatItIs: [
      "Think of Python variables as sticky notes attached to objects. Assignment attaches a name to an object, and multiple names can point to the same object.",
      "`==` asks: do these two sticky notes point to objects that consider themselves equal? It calls `__eq__`, which can be arbitrarily expensive. `is` asks: are these two sticky notes attached to the exact same object? It compares identity directly, with no method dispatch.",
    ],
    howItWorks: [
      "Assignment binds a name to an object in a namespace. Rebinding attaches the name to a different object and leaves the original object unchanged.",
      ...
      "User types can override `__eq__` to define custom equality. Identity checks bypass `__eq__` and compare the two object identities directly.",
    ],
    whenToUseIt: [
      ...
      "Audit aliasing carefully when mutating shared containers. The alias assignment creates the risk before the mutation line exposes it.",
    ],
    takeaways: [
      "`is` and `==` answer different questions: identity versus value equality. Choose the operator that matches the question.",
  ...
  "language-parameters": {
    ...
    whatItIs: [
      "Think of a signature like a physical key: only a correctly cut key enters the lock. Python's `/` and `*` markers cut your function's key such that wrong calls are rejected before the body runs.",
      "Positional-only (`/`) means those parameters accept positional arguments only, which lets you rename the parameter later without breaking anyone. Keyword-only (`*`) means callers must name those arguments, eliminating position confusion with booleans and flags.",
    ],
    howItWorks: [
      ...
      "Parameters before `/` are positional-only, so `connect(host=value)` raises TypeError.",
      "Parameters after `*` are keyword-only, so the call site must name them and avoid `connect(db, 5432, true, false)` ambiguity.",
    ],
    whenToUseIt: [
  ...
  "language-mutable-defaults": {
    boardIntro: "You wrote `def add_user(name, users=[])`, called it twice, and the second call had both users. The shared list came from definition-time evaluation.",
    whatItIs: [
    ...
    howItWorks: [
      ...
      "The fix: use `None` as the default and allocate inside the function body. That moves allocation from `def` time to call time, giving each call a fresh list.",
    ],
    whenToUseIt: [
      ...
      "Review helper APIs and decorators carefully. Convenience wrappers can hide this bug well.",
    ],
    takeaways: [
      "Definition time is when defaults are evaluated.",
      "The sentinel pattern (`if x is None: x = []`) is the standard fix.",
      "Shared state belongs in an explicit object rather than a hidden default.",
    ],
  ...
  "language-decorators-closures": {
    ...
    whatItIs: [
      "Think of a decorator like a mailing service: you hand it your letter (the original function), they put it in a new envelope with extra services (logging, timing, retries), and hand you back the new envelope. You keep the same name while calling the replacement callable.",
    ...
    howItWorks: [
      ...
      "`functools.wraps` copies common metadata (`__name__`, `__doc__`, `__module__`, annotations, and more) and sets `__wrapped__`. That lets introspection tools recover the original signature; without it, they see the wrapper's metadata.",
    ],
    whenToUseIt: [
    ...
    takeaways: [
      "Decorators run at import time while the function is being defined.",
      "`functools.wraps` preserves introspection, so production decorators should use it.",
  ...
  "language-type-hints": {
    ...
    whatItIs: [
      "Think of type hints like a blueprint annotation: \"this wall is load-bearing.\" The blueprint carries the information, while the wall keeps standing without checking the annotation every time someone leans on it. Type hints describe intent for readers and tools.",
    ...
    takeaways: [
      ...
      "Static validation and runtime validation are separate concerns, and production systems often need both.",
  ...
  "language-bytecode-dis": {
    boardIntro: "Two snippets can produce the same result while one emits twice the opcodes. Bytecode shows you why.",
    whatItIs: [
      ...
      "Different source patterns produce different opcode sequences. A list comprehension might use `LIST_APPEND` while a manual loop uses explicit loads plus a call opcode (`CALL` in current CPython). Same result, different interpreter cost.",
    ],
    howItWorks: [
      ...
      "The interpreter dispatches opcodes in a loop. CPython 3.11+ can quicken bytecode and specialize hot operations at runtime; `dis` can show either the baseline or adaptive form depending on options.",
      "Opcode count tells only part of the story. Some opcodes (like `LOAD_FAST`) are a single array lookup; others (like `LOAD_ATTR`) may trigger descriptor protocol, `__getattr__`, or property calls.",
    ],
    whenToUseIt: [
      ...
      "Pair opcode inspection with timing real workloads. Bytecode explains shape; measurements still decide.",
    ],
    takeaways: [
      "Bytecode is a diagnostic layer for code shape. Benchmarks decide performance.",
  ...
  "language-dunder-methods": {
    ...
    whatItIs: [
      "Think of dunder methods like electrical outlets in a wall: the prongs have a specific shape for a reason. `__eq__` fits the equality socket. `__iter__` fits the iteration socket. Wire the protocol correctly, and the language can plug your type into the right operations.",
    ...
    howItWorks: [
      "The interpreter looks up special methods on the **type** rather than the instance. `type(obj).__eq__(obj, other)` is the relevant lookup path, which is why special methods still work when an instance attribute shadows the same name.",
      "Some operations have fallback paths. Iteration falls back to `__getitem__` with increasing integer indexes when `__iter__` is absent. `__bool__` falls back to `__len__`.",
      "Method combinations matter: `__eq__` and `__hash__` must be designed together. If a class defines `__eq__` without `__hash__`, Python sets `__hash__ = None`, so `hash(obj)` raises TypeError until a hash policy is supplied.",
    ],
    whenToUseIt: [
      "Implement a special method only when your type genuinely models that protocol.",
      "Keep semantics unsurprising. `__eq__` should compare values without unrelated side effects such as logging to a file.",
    ...
    takeaways: [
      "Special methods define behavior contracts that go beyond operator overloads.",
      "Protocol correctness matters more than syntactic cleverness.",
      "Hashing, equality, and mutability must be designed as one contract.",
    ],
  ...
  "runtime-del-gc": {
    ...
    whatItIs: [
      "Think of `del` like taking a sticker off a filing cabinet. The cabinet stays in place while another sticker still points to it. Only when the last sticker is gone can the cabinet be recycled.",
    ...
    howItWorks: [
      ...
      "Reference counting handles direct lifetimes. Cycles (two objects referencing each other with no external references) need CPython's cyclic garbage collector, which runs periodically to detect and collect unreachable cycles.",
      "`__del__` finalizers complicate collection order and can run at awkward times, but modern Python can collect many cycles that contain finalizers. `gc.garbage` is mostly for debug retention or extension types with legacy finalization hooks. Prefer context managers for production resource cleanup.",
    ],
    whenToUseIt: [
      ...
      "Treat `__del__` as a last resort with narrow constraints. Prefer context managers for production resource management.",
    ],
    takeaways: [
      "`del` changes bindings, not object identity.",
      "Reference counting is immediate in CPython. Other Python implementations may reclaim objects at different times.",
  ...
  "dict-hash-tables": {
    boardIntro: "A dict lookup is O(1) on average. A list search is O(n). Hashing creates the speedup, and hashability is a correctness rule.",
    whatItIs: [
      "Think of a dict like a coat check. You hand over your coat and get a numbered ticket. When you come back, the ticket points to the right place. Dict lookup works the same way: it hashes the key and uses the hash to find where to look.",
      "The tradeoff is memory. A hash table keeps empty slots so lookups stay fast. Pack the table too dense and collisions spike. CPython resizes tables before they get too crowded, keeping lookups fast at the cost of spare space.",
    ],
    howItWorks: [
      ...
      "If occupied by a different key (a collision), Python probes forward using perturbed pseudo-random probing until it finds either a matching key or an empty slot. The amortized cost stays O(1) when hashes distribute well.",
      "CPython 3.6 introduced a compact layout with a sparse index array and a dense entries array. Insertion order became a language guarantee in Python 3.7; before that, it was a CPython implementation detail.",
    ],
    whenToUseIt: [
      "Use dicts when you need to look up values by a key, not by position.",
      "Prefer sets when only membership matters. They use hash-table lookup without value storage.",
      "Keep keys immutable for their lifetime in the dict. A key whose hash changes becomes unreachable, leaving the entry stranded until overwritten or until the dict dies.",
    ],
    takeaways: [
      "O(1) lookup is amortized. Degenerate hashing can collapse it to O(n).",
      "Memory overhead is the price of speed: sparse tables beat dense scans.",
      "Hashability is a correctness invariant rather than a performance hint.",
    ],
    tryItOutCode: "cache = {(\"user\", 42): \"active\"}\nprint(cache[(\"user\", 42)])\ntry:\n    cache[[\"user\", 42]] = \"bad\"\nexcept TypeError as exc:\n    print(type(exc).__name__)",
    visualKind: "hash-probe",
    simulationKind: "hash",
    simulationTitle: "Lookup Time vs Table Pressure",
    reminder: "Hashability is required.",
  },
  "dict-setdefault": {
    ...
    howItWorks: [
      ...
      "`collections.defaultdict(factory)` overrides `__missing__`, which `__getitem__` calls for a missing key. The factory is called with no arguments to produce the default value.",
      "`setdefault` evaluates its default argument eagerly before the lookup. `defaultdict` calls the factory lazily when a key is actually missing.",
    ],
    whenToUseIt: [
      ...
      "Use ordinary lookup when a missing key means a business error, so the `KeyError` surfaces.",
    ],
    takeaways: [
  ...
  "sets-membership-views": {
    ...
    whatItIs: [
      "Think of a set like a bouncer with perfect memory. You show up, they check your face and answer immediately. The hash table lets the bouncer jump to the right spot instead of scanning the guest list line by line.",
      "Dictionary keys are also hashable, so `dict_keys` and hashable `dict_items` can participate in set operations without building a new set. `payload.keys() - allowed_keys` is a set difference on a live view with no intermediate `set()` allocation.",
    ],
    howItWorks: [
    ...
    takeaways: [
      ...
      "Set algebra (`|`, `&`, `-`, `^`) can replace slower nested loops with direct hash-table operations.",
  ...
  "memory-container-comparison": {
    ...
    whatItIs: [
      "Think of containers like storage units: `list` is a shelf where every item is in its own box (PyObject reference). `tuple` is the same shelf welded shut, with no adding or removing items. `set` is a warehouse with an instant-lookup catalog (hash table) and spare floor space for speed. `array.array` is a pallet of identical crates packed tight, no individual boxes at all.",
    ...
    howItWorks: [
      ...
      "Sets use a hash table with spare slots. That empty space is the price of O(1) average membership.",
    ...
    takeaways: [
      ...
      "Measure the dominant workload and verify folklore against data.",
  ...
  "memory-list-alternatives": {
    boardIntro: "Lists are flexible, and that flexibility has a cost. For numeric data, queues, and binary buffers, specialized containers skip the overhead.",
    whatItIs: [
      "A list of floats is an array of PyObject pointers pointing to individual float objects scattered on the heap. A packed C-double buffer needs a different container, such as `array.array('d')`.",
    ...
    howItWorks: [
      "`array.array` packs C primitives contiguously in a single memory allocation. Appending may reallocate, and elements live inside the array storage rather than as separate Python float objects. Memory per element: 8 bytes for `'d'` vs ~32 bytes for a list of floats.",
      "`deque` uses fixed-size blocks linked together. Appending to either end is O(1) amortized because existing elements stay in place, unlike `list.insert(0, x)` or `list.pop(0)`.",
      "`memoryview` exposes the buffer protocol. Slice assignment like `view[2:4] = b'XY'` writes directly into the underlying buffer without allocating a new bytes object.",
    ],
    whenToUseIt: [
  ...
  "memory-tuples-lists": {
    boardIntro: "A tuple allocates exactly. A list over-allocates. That extra capacity is the deliberate price of efficient append.",
    whatItIs: [
      "Think of tuple like a fixed-size parking lot: you buy exactly N spaces and that is what you get. A list is like a parking lot with expansion land bought alongside: you pay for a few extra empty spaces so that when a new car arrives, you avoid rebuilding the whole lot.",
    ...
    howItWorks: [
      "Lists store a pointer to a dynamic array of `PyObject*` plus allocated capacity and current length. When `len == capacity`, CPython grows the allocation by roughly 1.125x for large lists. The old array is freed; existing references are copied.",
      ...
      "Tuple immutability can make tuples usable as dict keys and set members when every contained element is hashable. Lists are mutable and unhashable, so hash-key roles belong to other containers.",
    ],
    whenToUseIt: [
      ...
      "Choose tuples when the contract is \"fixed shape.\" Small size alone is a weak reason to replace a list.",
    ],
    takeaways: [
      "Tuple's advantage is fixed shape and exact allocation.",
      "List's spare capacity is deliberate because it enables efficient growth.",
  ...
  "memory-iterables": {
    ...
    whatItIs: [
      "Think of a list like a book: every page exists, bound together, ready to flip to any page instantly. An iterable is like reading a scroll from a spool: you see one section at a time, and once read, the text is behind the spool unless you recorded it.",
    ...
    howItWorks: [
      ...
      "Generator functions (`def f(): yield x`) compile to generator objects. Each call to `next(gen)` resumes the frame, runs to the next `yield`, produces a value, and suspends again. The frame stores locals and the current instruction pointer, so memory grows with frame state rather than data volume.",
    ...
    whenToUseIt: [
      ...
      "Document one-shot iterables clearly so callers avoid accidental double consumption.",
    ],
    takeaways: [
      "Laziness trades replayability for memory savings.",
      "A generator's memory cost follows the frame rather than the data volume.",
      "An iterable contract gives callers fewer guarantees than a container contract, so document replay and traversal behavior.",
    ],
  ...
  "classes-data-builders": {
    boardIntro: "NamedTuple, typing.NamedTuple, and @dataclass all build records. Each one makes a different tradeoff.",
    whatItIs: [
      ...
      "NamedTuple records are tuples, so they support indexing, unpacking, and pattern matching. @dataclass instances are regular class instances with generated `__init__`, `__repr__`, `__eq__`, optional ordering methods, and hash behavior determined by `eq`, `frozen`, and `unsafe_hash`.",
    ],
    howItWorks: [
    ...
    whenToUseIt: [
      ...
      "Choose between them by capabilities and constraints, not by the fact that both hold fields.",
    ],
    takeaways: [
  ...
  "classes-dataclass-fields": {
    boardIntro: "One @dataclass decorator can generate __init__, __repr__, __eq__, ordering methods, and sometimes __hash__. The field options decide how, and that is where the design lives.",
    whatItIs: [
      "Think of dataclass field options like a car's options package: `frozen` is like the child lock (no one can change the seats after delivery). `order` is like having a VIN comparator, so you can sort by all fields in order. `default_factory` is like a custom trunk organizer that is built fresh for each car instead of sharing one across the assembly line.",
      "Each option controls a different axis of generated behavior. The flags change the class contract. `frozen=True` makes the instance hashable if `__eq__` is also generated. `order=True` generates `<`, `<=`, `>`, `>=`.",
    ],
    howItWorks: [
      "The decorator inspects annotations and field configuration to generate `__init__`, `__repr__`, `__eq__`, and, when requested, `__lt__`, `__le__`, `__gt__`, and `__ge__`. `__hash__` follows the dataclass safety rules for `eq`, `frozen`, and `unsafe_hash`.",
      "`default_factory=list` stores a callable. Each instance construction calls `list()` to produce a fresh default. This uses the same allocation-per-call idea as the standard fix for function mutable defaults.",
      "`slots=True` generates `__slots__` from the field names, eliminating `__dict__`. Instances use less memory and attribute access is faster. Dynamic attribute assignment (`obj.extra = x`) is disabled.",
    ],
    whenToUseIt: [
      "Use explicit field configuration whenever defaults or generated comparisons are part of the API.",
      "Use `slots=True` when you have many instances and dynamic attributes are unnecessary.",
      "Keep `__post_init__` narrow. Use it for validation or derived fields, and keep unrelated side effects elsewhere.",
    ],
    takeaways: [
      ...
      "Generated methods should match the domain semantics exactly. Write the method yourself when the contract needs custom behavior.",
    ],
  ...
  "runtime-gil-performance": {
    boardIntro: "Eight threads waiting on HTTP responses can finish faster than one. Eight threads running pure Python CPU loops still share one bytecode lane while the GIL is enabled.",
    whatItIs: [
      "Think of the GIL like a single bathroom key in an office building. Only one person can use the bathroom at a time. CPU-bound Python threads keep asking for that key. I/O-bound threads spend most of their time waiting somewhere else, so their waits overlap efficiently.",
      "The GIL is a mutex on the CPython interpreter that prevents two threads from executing Python bytecode simultaneously. I/O overlap still works because many blocking I/O operations release the GIL while waiting.",
    ],
    howItWorks: [
      "Only one thread can execute Python bytecode at a time while the GIL is enabled. CPython switches between runnable threads using a time-based switch interval, and a thread that performs blocking I/O normally releases the GIL while the kernel waits.",
      ...
      "Free-threaded CPython builds (PEP 703, Python 3.13+) can run with the GIL disabled. Separately, Python 3.12 added a per-interpreter GIL for subinterpreters (PEP 684).",
    ],
    whenToUseIt: [
      "Use threads freely for I/O orchestration because the GIL is released during many blocking waits.",
    ...
    takeaways: [
      "The GIL constrains Python bytecode throughput while still leaving threads useful for I/O.",
      "Shared memory remains a thread advantage over processes because it avoids serialization overhead.",
    ...
    reminder: "CPU and I/O have different concurrency stories.",
  },
  "async-foundations-awaitables": {
    boardIntro: "`await` is a suspension point that pauses the coroutine so the event loop can run something else. Parallelism is a separate question.",
    whatItIs: [
      "Think of async like a chef with multiple orders: instead of standing at the stove stirring one pot until it boils (blocking), the chef starts the pot, turns to chop vegetables for another order, and checks the first pot periodically. The chef handles one action at a time while timers and I/O waits overlap.",
    ...
    howItWorks: [
      ...
      "Any blocking CPU work or synchronous I/O inside a coroutine stalls the entire loop because other tasks wait for the next suspension point.",
    ],
    whenToUseIt: [
      ...
      "Model ownership and cancellation up front. Async code without teardown leaks resources.",
    ],
    takeaways: [
      ...
      "Await boundaries are the unit of cooperation because every `await` is an opportunity for the loop.",
    ],
  ...
  "async-context-backpressure": {
    ...
    whenToUseIt: [
      ...
      "Watch queue growth as a signal of overload. Unbounded growth means demand has exceeded capacity.",
    ],
    takeaways: [
      ...
      "Backpressure is a design responsibility that must be placed where the resource bottleneck lives.",
    ],
  ...
  "async-servers-services": {
    boardIntro: "A server spends much of its life waiting for connections, socket data, and output buffers to drain. Async makes that waiting explicit and returns control to the loop between waits.",
    whatItIs: [
      "Think of an async server like a busy receptionist who handles calls one at a time. They start a call, ask a question, and while waiting for the answer, they start the next call. Waiting is shared across calls.",
    ...
    howItWorks: [
      ...
      "Slow peers are a fact of network programming. A handler that writes to a slow client can pause at `drain()`, while other connections proceed.",
    ],
    whenToUseIt: [
      ...
      "Move CPU-heavy parsing or synchronous database calls out of handler coroutines and into a thread or process.",
    ],
    takeaways: [
      "Service correctness includes flow control and deterministic cleanup.",
      "Drain boundaries are operationally important because a stalled write blocks one handler rather than the whole server.",
  ...
  "async-iterators-generators": {
    ...
    whatItIs: [
      "Think of an async generator like a vending machine that restocks each item just before dispensing. You wait for one item, receive it, and the machine restocks the next one while you eat the first.",
    ...
    whenToUseIt: [
      ...
      "Prefer them when downstream consumers can process items incrementally, reducing time-to-first-item and peak memory.",
      "Document whether the stream is replayable or one-shot. An exhausted async generator is spent.",
    ],
    takeaways: [
      "Streaming reduces both peak memory and time-to-first-item.",
      "The consumer participates in scheduling cost because each `async for` iteration is an await point.",
  ...
  "async-limits-type-hints": {
    boardIntro: "Async helps with I/O latency. CPU work still needs a separate parallelism strategy. Type hints tell you the shape, while measurements reveal scheduling cost.",
    whatItIs: [
      "Think of async like a highway carpool lane: it helps when everyone is waiting at toll booths (I/O). Slow CPU work still occupies the lane, so everyone behind it waits.",
      "CPU-heavy work inside a coroutine monopolizes the event loop. Other tasks run only after that bytecode reaches a suspension point, so even I/O waits behind the CPU work.",
    ],
    howItWorks: [
      "A coroutine that spends 5 seconds in a pure-Python loop without `await` blocks every other task on the same event loop. The loop needs a suspension point before it can schedule another task.",
      "`Awaitable`, `AsyncIterator`, and `AsyncGenerator` are type-level protocols. They tell callers and checkers which values are coroutines versus concrete results. A type checker still leaves scheduling behavior for runtime review.",
    ...
    takeaways: [
      "Async optimizes for I/O overlap. Treat it as a targeted concurrency tool.",
      "Type clarity matters more when control flow is indirect (callbacks, futures, coroutines).",
      "Measure event loop lag and backlog alongside raw task counts.",
    ],
  ...
  "asyncio-task-groups": {
    boardIntro: "`TaskGroup` gives related tasks one failure boundary. `gather()` waits for awaitables, while TaskGroup owns sibling cancellation and cleanup.",
    whatItIs: [
      "Think of `TaskGroup` like a tour guide with a group of hikers: if one hiker falls, the guide calls everyone back to base. Everyone is accounted for. A loose `gather()` call feels more like handing everyone a map and tracking results afterward.",
    ...
    howItWorks: [
      ...
      "`TaskGroup` gives tasks an owner and a shared failure boundary. Fan-out still needs its own limit: if you start 10,000 tasks inside a group, all 10,000 are scheduled. Use a semaphore alongside the group to limit concurrency.",
    ],
    whenToUseIt: [
  ...
  "production-stdlib-logging": {
    ...
    whatItIs: [
      "Think of `logging` like a postal sorting facility: each envelope (log record) has a destination address (logger name), a priority stamp (level), and routing instructions (handlers, formatters). The application drops the envelope in the mail slot; the logging configuration handles routing.",
    ...
    howItWorks: [
      "Logger names form a hierarchy via dot-separated naming. A logger named `orders.payment` is a child of `orders`. Child loggers propagate to parent handlers by default, so you can configure the root logger and let all children inherit.",
